...
void MFRC522::PCD_Init() {
	...
	DigitalInOut MFRC522_chipSelectPin(_chipSelectPin);
	DigitalInOut MFRC522_resetPowerDownPin(_resetPowerDownPin);
	MFRC522_chipSelectPin.output();
	MFRC522_chipSelectPin = 1;
	
	// If a valid pin number has been set, pull device out of power down / reset state.
	if (_resetPowerDownPin != UNUSED_PIN) {
		// First set the resetPowerDownPin as digital input, to check the MFRC522 power down mode.
		MFRC522_resetPowerDownPin.input();
	
		if (MFRC522_resetPowerDownPin == 0) {	// The MFRC522 chip is in power down mode.
			MFRC522_resetPowerDownPin.output();		// Now set the resetPowerDownPin as digital output.
			MFRC522_resetPowerDownPin = 0;		// Make sure we have a clean LOW state.
			wait_us(2);				// 8.8.1 Reset timing requirements says about 100ns. Let us be generous: 2μsl
			MFRC522_resetPowerDownPin = 1; 	// Exit power down mode. This triggers a hard reset.
 ...
 * Initializes the MFRC522 chip.
 */
void MFRC522::PCD_Init(	PinName resetPowerDownPin	///<  pin connected to MFRC522's reset and power down input (Pin 6, NRSTPD, active low)
					) {
	PCD_Init(PA_4, resetPowerDownPin); // SPI1 SS is defined in NEUCLEO F130RB board
} // End PCD_Init()
...
void MFRC522::PCD_SoftPowerUp(){
	...
	while( duration_cast<milliseconds>(ms_timer.elapsed_time()).count() <= timeout ){ // set timeout to 500 ms 
